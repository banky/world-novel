...
const StoryContent = () => {
  ...
  const [openSentence, setOpenSentence] = useState<number | undefined>(
    undefined
  );
...
const VoteDialog = ({
  ...
  sentences: SentenceStructOutput[] | undefined;
  ...
  const voteMutation = useMutation(
    async () => {
      const transaction = await worldNovel.voteOnSentence(sentenceIndex, count);
      await transaction.wait();
    },
    {
      onSuccess: async () => {
        await queryClient.invalidateQueries("currentSentences");
        dismissAndResetCount();
      },
    }
  );
  ...
  return (
    <Dialog
      isOpen={isOpen}
      onDismiss={dismissAndResetCount}
      aria-label="Vote on sentence"
    >
